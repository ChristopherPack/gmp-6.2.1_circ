...
static mp_limb_t mpn_mul_fft_internal (mp_ptr, mp_size_t, int, mp_ptr *,
				       ...
				       mp_size_t, mp_size_t, mp_ptr, int);
...
static inline void
mpn_fft_add_sub_modF_2 (mp_ptr A0, mp_ptr Ai, mp_srcptr tp, mp_size_t n)
{
  mp_limb_t cyas, c, x;


  cyas = mpn_add_n_sub_n (A0, Ai, Ai, tp, n);
...
static inline void
mpn_fft_sub_add_modF (mp_ptr A0, mp_ptr Ai, mp_srcptr tp, mp_size_t n)
{
  mp_limb_t cyas, c, x;

  cyas = mpn_add_n_sub_n (A0, Ai, tp, Ai, n);
...
/*
 * Version of circ alg with normal multiplication at the end.
 * Multiplication directly copied from mul_modf_k
 */
static void
mpn_mul_circ_mult_basic(mp_ptr *a, mp_ptr *b, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_limb_t cc, mp_size_t n2, mp_ptr tp, mp_ptr tpn) {
    mp_size_t halfN = N >> 1;
    for (int i = 0; i < halfN; i++) {
        mpn_fft_mul_2exp_modF(tmp, a[halfN + i], f, p);

#if HAVE_NATIVE_mpn_add_n_sub_n
        mpn_fft_add_sub_modF (a[i], a[halfN+i], tmp, p);
#else
        mpn_fft_sub_modF (a[halfN+i], a[i], tmp, p);
        mpn_fft_add_modF(a[i], a[i], tmp, p);
#endif


        mpn_fft_mul_2exp_modF(tmp, b[halfN+i], f, p);
#if HAVE_NATIVE_mpn_add_n_sub_n
        mpn_fft_add_sub_modF (b[i], b[halfN+i], tmp, p);
#else
        mpn_fft_sub_modF (b[halfN+i], b[i], tmp, p);
        mpn_fft_add_modF(b[i], b[i], tmp, p);
#endif
    }

    if(N == 2) {
        mpn_mul_n (tp, b[0], a[0], p);
        if (a[0][p] != 0)
            cc = mpn_add_n (tpn, tpn, b[0], p);
        else
            cc = 0;
        if (b[0][p] != 0)
            cc += mpn_add_n (tpn, tpn, a[0], p) + a[0][p];
        if (cc != 0)
        {
        ...
        a[0][p] = mpn_sub_n (a[0], tp, tpn, p) && mpn_add_1 (a[0], a[0], p, CNST_LIMB(1));


        mpn_mul_n (tp, b[1], a[1], p);
        if (a[1][p] != 0)
            cc = mpn_add_n (tpn, tpn, b[1], p);
        else
            cc = 0;
        if (b[1][p] != 0)
            cc += mpn_add_n (tpn, tpn, a[1], p) + a[1][p];
        if (cc != 0)
        {
            cc = mpn_add_1 (tp, tp, n2, cc);
            /* If mpn_add_1 give a carry (cc != 0),
        ...
        a[1][p] = mpn_sub_n (a[1], tp, tpn, p) && mpn_add_1 (a[1], a[1], p, CNST_LIMB(1));
    }
    else {
        mpn_mul_circ_mult_basic(a, b, halfN, f >> 1, p, tmp, cc, n2, tp, tpn);
        mpn_mul_circ_mult_basic(a+halfN, b+halfN, halfN, (p * GMP_NUMB_BITS + f) >> 1, p, tmp, cc, n2, tp, tpn);
    }
    for (int i = 0; i < halfN; i++) {
        mpn_fft_sub_modF(tmp, a[i], a[halfN+i], p);
        mpn_fft_add_modF(a[i], a[i], a[halfN+i], p);
        mpn_fft_mul_2exp_modF(a[halfN+i], tmp, f, p);
    }
}
 ...
 */
static void
mpn_mul_circ_mult_basic_SQR(mp_ptr *a, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_limb_t cc, mp_size_t n2, mp_ptr tp, mp_ptr tpn) {
    mp_size_t halfN = N >> 1;
    for (int i = 0; i < halfN; i++) {
        mpn_fft_mul_2exp_modF(tmp, a[halfN + i], f, p);
    ...
    if(N == 2) {
        ...
        mpn_mul_circ_mult_basic_SQR(a, halfN, f >> 1, p, tmp, cc, n2, tp, tpn);
        mpn_mul_circ_mult_basic_SQR(a+halfN, halfN, (p * GMP_NUMB_BITS + f) >> 1, p, tmp, cc, n2, tp, tpn);
    }
    for (int i = 0; i < halfN; i++) {
        mpn_fft_sub_modF(tmp, a[i], a[halfN+i], p);
        mpn_fft_add_modF(a[i], a[i], a[halfN+i], p);
...
/*
 * Version of circ alg with normal multiplication at the end.
 * First variation optimized for root = -1
 * There is not optimization for b
 * Multiplication directly copied from mul_modf_k
 */
static void
mpn_mul_circ_mult_basic_first(mp_ptr *a, mp_ptr *b, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_limb_t cc, mp_size_t n2, mp_ptr tp, mp_ptr tpn) {
    mp_size_t halfN = N >> 1;
    mp_limb_t cy;
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (a[halfN+i], a[i], a[halfN+i], a[i], p + 1) & 1;
#else
        MPN_COPY (tmp, a[halfN+i], p + 1);
        mpn_add_n (a[halfN+i], a[halfN+i], a[i], p + 1);
        cy = mpn_sub_n (a[i], tmp, a[i], p + 1);
#endif
        if (a[halfN+i][p] > 1)
            a[halfN+i][p] = 1 - mpn_sub_1 (a[halfN+i], a[halfN+i], p, a[halfN+i][p] - 1);
        if (cy)
            a[i][p] = mpn_add_1 (a[i], a[i], p, ~a[i][p] + 1);

#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (b[halfN+i], b[i], b[halfN+i], b[i], p + 1) & 1;
#else
        MPN_COPY (tmp, b[halfN+i], p + 1);
        mpn_add_n (b[halfN+i], b[halfN+i], b[i], p + 1);
        cy = mpn_sub_n (b[i], tmp, b[i], p + 1);
#endif
        if (b[halfN+i][p] > 1)
            b[halfN+i][p] = 1 - mpn_sub_1 (b[halfN+i], b[halfN+i], p, b[halfN+i][p] - 1);
        if (cy)
            b[i][p] = mpn_add_1 (b[i], b[i], p, ~b[i][p] + 1);
    }
    ...
    if(N == 2) {
        ...
        mpn_mul_circ_mult_basic(a, b, halfN, f >> 1, p, tmp, cc, n2, tp, tpn);
        mpn_mul_circ_mult_basic_first(a+halfN, b+halfN, halfN, f, p, tmp, cc, n2, tp, tpn);
    }
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (a[i], a[halfN+i], a[halfN+i], a[i], p + 1) & 1;
#else
        MPN_COPY (tmp, a[i], p + 1);
        mpn_add_n (a[i], a[i], a[halfN+i], p + 1);
        cy = mpn_sub_n (a[halfN+i], a[halfN+i], tmp, p + 1);
#endif
        if (a[i][p] > 1)
            a[i][p] = 1 - mpn_sub_1 (a[i], a[i], p, a[i][p] - 1);
        if (cy)
            a[halfN+i][p] = mpn_add_1 (a[halfN+i], a[halfN+i], p, ~a[halfN+i][p] + 1);
    }
}
 ...
 * First variation optimized for root = -1
 * There is not optimization for b
 * Multiplication directly copied from original mul_modf_k
 */
static void
mpn_mul_circ_mult_basic_SQR_first(mp_ptr *a, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_limb_t cc, mp_size_t n2, mp_ptr tp, mp_ptr tpn) {
    mp_size_t halfN = N >> 1;
    mp_limb_t cy;
    for (int i = 0; i < halfN; i++) {
    ...
    if(N == 2) {
        ...
        mpn_mul_circ_mult_basic_SQR(a, halfN, f >> 1, p, tmp, cc, n2, tp, tpn);
        mpn_mul_circ_mult_basic_SQR_first(a+halfN, halfN, f, p, tmp, cc, n2, tp, tpn);
    }
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (a[i], a[halfN+i], a[halfN+i], a[i], p + 1) & 1;
...
/*
 * circ alg with fft recursion on multiplication.
 * fft recursion code directly copied from mul_modf_k
 */
static void
mpn_mul_circ_mult_basic_internal(mp_ptr *a, mp_ptr *b, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_ptr A, mp_ptr B, mp_ptr *Ap, mp_ptr *Bp, mp_size_t K2, mp_size_t nprime2, mp_size_t Mp2, mp_size_t l, mp_ptr T, int sqr, int k) {
    mp_size_t halfN = N >> 1;
    for (int i = 0; i < halfN; i++) {
        mpn_fft_mul_2exp_modF(tmp, a[halfN + i], f, p);
    ...
    if (N == 2) {
        mp_limb_t cya;
        mpn_fft_normalize(a[0], p);
        mpn_fft_normalize(b[0], p);
        mpn_mul_fft_decompose(A, Ap, K2, nprime2, a[0], (l << k) + 1, l, Mp2, T);
        mpn_mul_fft_decompose(B, Bp, K2, nprime2, b[0], (l << k) + 1, l, Mp2, T);

        cya = mpn_mul_fft_internal(a[0], p, k, Ap, Bp, A, B, nprime2,l, Mp2, T, sqr);
        a[0][p] = cya;

        mpn_fft_normalize(a[1], p);
        mpn_fft_normalize(b[1], p);
        mpn_mul_fft_decompose(A, Ap, K2, nprime2, a[1], (l << k) + 1, l, Mp2, T);
        mpn_mul_fft_decompose(B, Bp, K2, nprime2, b[1], (l << k) + 1, l, Mp2, T);

        cya = mpn_mul_fft_internal(a[1], p, k, Ap, Bp, A, B, nprime2, l, Mp2, T, sqr);
        a[1][p] = cya;
    } else {
        mpn_mul_circ_mult_basic_internal(a, b, halfN, f >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
        mpn_mul_circ_mult_basic_internal(a + halfN, b + halfN, halfN, (p * GMP_NUMB_BITS + f) >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
    }
    for (int i = 0; i < halfN; i++) {
        mpn_fft_sub_modF(tmp, a[i], a[halfN+i], p);
        mpn_fft_add_modF(a[i], a[i], a[halfN+i], p);
 ...
 */
static void
mpn_mul_circ_mult_basic_internal_SQR(mp_ptr *a, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_ptr A, mp_ptr B, mp_ptr *Ap, mp_ptr *Bp, mp_size_t K2, mp_size_t nprime2, mp_size_t Mp2, mp_size_t l, mp_ptr T, int sqr, int k) {
    mp_size_t halfN = N >> 1;
    for (int i = 0; i < halfN; i++) {
        mpn_fft_mul_2exp_modF(tmp, a[halfN + i], f, p);
    ...
    if (N == 2) {
        ...
        mpn_mul_circ_mult_basic_internal_SQR(a, halfN, f >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
        mpn_mul_circ_mult_basic_internal_SQR(a + halfN, halfN, (p * GMP_NUMB_BITS + f) >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
    }
    for (int i = 0; i < halfN; i++) {
        mpn_fft_sub_modF(tmp, a[i], a[halfN+i], p);
        mpn_fft_add_modF(a[i], a[i], a[halfN+i], p);
...
/*
 * circ alg with fft recursion on multiplication.
 * First version optimized for root = -1
 * There is no optimization for b
 * fft recursion code directly copied from mul_modf_k
 */
static void
mpn_mul_circ_mult_basic_internal_first(mp_ptr *a, mp_ptr *b, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_ptr A, mp_ptr B, mp_ptr *Ap, mp_ptr *Bp, mp_size_t K2, mp_size_t nprime2, mp_size_t Mp2, mp_size_t l, mp_ptr T, int sqr, int k) {
    mp_size_t halfN = N >> 1;
    mp_limb_t cy;
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
    ...
    if (N == 2) {
        ...
        mpn_mul_circ_mult_basic_internal(a, b, halfN, f >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
        mpn_mul_circ_mult_basic_internal_first(a + halfN, b + halfN, halfN, f, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
    }
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (a[i], a[halfN+i], a[halfN+i], a[i], p + 1) & 1;
 ...
 */
static void
mpn_mul_circ_mult_basic_internal_SQR_first(mp_ptr *a, mp_size_t N, mp_size_t f, mp_size_t p, mp_ptr tmp, mp_ptr A, mp_ptr B, mp_ptr *Ap, mp_ptr *Bp, mp_size_t K2, mp_size_t nprime2, mp_size_t Mp2, mp_size_t l, mp_ptr T, int sqr, int k) {
    mp_size_t halfN = N >> 1;
    mp_limb_t cy;
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
    ...
    if (N == 2) {
        ...
        mpn_mul_circ_mult_basic_internal_SQR(a, halfN, f >> 1, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
        mpn_mul_circ_mult_basic_internal_SQR_first(a + halfN, halfN, f, p, tmp, A, B, Ap, Bp, K2, nprime2, Mp2, l, T, sqr, k);
    }
    for (int i = 0; i < halfN; i++) {
#if HAVE_NATIVE_mpn_add_n_sub_n
        cy = mpn_add_n_sub_n (a[i], a[halfN+i], a[halfN+i], a[i], p + 1) & 1;
...
/*
 * We need these two versions because we can't use mul_modF_K since each call calculates these values, which is not optimal.
 */
static void
mpn_mul_circ_base(mp_ptr *Ap, mp_ptr *Bp, mp_size_t nprime, mp_size_t K, mp_ptr tmp, int sqr) {
    TMP_DECL;
    TMP_MARK;
    if (nprime >= (sqr ? SQR_FFT_MODF_THRESHOLD : MUL_FFT_MODF_THRESHOLD))
    {
        ...
        int k2;
        mp_ptr *Ap2, *Bp2, *temp, A2, B2, T;

        k2 = mpn_fft_best_k (nprime, sqr);
        K2 = (mp_size_t) 1 << k2;
        ASSERT_ALWAYS((nprime & (K2 - 1)) == 0);
        maxLK = (K2 > GMP_NUMB_BITS) ? K2 : GMP_NUMB_BITS;
        M2 = nprime * GMP_NUMB_BITS >> k2;
        l = nprime >> k2;
        Nprime2 = ((2 * M2 + k2 + 2 + maxLK) / maxLK) * maxLK;
        ...
        ASSERT_ALWAYS(nprime2 < nprime); /* otherwise we'll loop */

        Mp2 = Nprime2 >> k2;

        Ap2 = TMP_BALLOC_MP_PTRS (K2);
        Bp2 = TMP_BALLOC_MP_PTRS (K2);
        A2 = TMP_BALLOC_LIMBS (2 * (nprime2 + 1) << k2);
        T = TMP_BALLOC_LIMBS (2 * (nprime2 + 1));
        B2 = A2 + ((nprime2 + 1) << k2);
        TRACE(gmp_printf ("recurse: %ldx%ld limbs -> %ld times %ldx%ld (%1.2f)\n", nprime, nprime, K2, nprime2, nprime2, 2.0*(double)nprime/nprime2/K2));
        if(sqr) {
            mpn_mul_circ_mult_basic_internal_SQR_first(Ap, K, nprime * GMP_NUMB_BITS, nprime, tmp, A2, B2, Ap2, Bp2, K2, nprime2, Mp2, l, T, sqr, k2);
        } else {
            mpn_mul_circ_mult_basic_internal_first(Ap, Bp, K, nprime * GMP_NUMB_BITS, nprime, tmp, A2, B2, Ap2, Bp2, K2, nprime2, Mp2, l, T, sqr, k2);
        }
    }
    else
    {
        mp_limb_t cc;
        mp_size_t n2 = 2 * nprime;
        mp_ptr tp = TMP_BALLOC_LIMBS(n2);
        mp_ptr tpn = tp + nprime;
        if(sqr) {
            mpn_mul_circ_mult_basic_SQR_first(Ap, K, nprime * GMP_NUMB_BITS, nprime, tmp, cc, n2, tp, tpn);
        } else {
            mpn_mul_circ_mult_basic_first(Ap, Bp, K, nprime * GMP_NUMB_BITS, nprime, tmp, cc, n2, tp, tpn);
        }
    }
    TMP_FREE;
}
...
mpn_mul_fft_internal (mp_ptr op, mp_size_t pl, int k,
		      ...
		      mp_size_t nprime, mp_size_t l, mp_size_t Mp, mp_ptr T, int sqr)
{
  ...
  mpn_mul_circ_base(Ap, Bp, nprime, K, T, sqr);
	     ...
	     int k)
{
  ...
  //mp_size_t pla;
  //pla = l * (K - 1) + nprime + 1; /* number of required limbs for p */
  h = mpn_mul_fft_internal (op, pl, k, Ap, Bp, A, B, nprime, l, Mp, T, sqr);
